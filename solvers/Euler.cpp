...
//    std::cout << jv << std::endl << std::endl;
    ...
    for (int i = 0; i < dy.size(); i += 3) {
        ...
        newState[si + 3] = oldState[si + 3] + dy.coeff(i+0);        // Update velocity
        newState[si + 4] = oldState[si + 4] + dy.coeff(i+1);
        newState[si + 5] = oldState[si + 5] + dy.coeff(i+2);
    }
